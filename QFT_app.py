...
default_state_str = ", ".join(f"{x:.1f}" for x in default_state)
custom_state_input = st.text_input(
    f"输入 {2**num_qubits} 个复数值(用逗号分隔):",
    value=default_state_str
)

# 解析用户输入的初态
try:
    custom_state = np.array([complex(x.strip()) for x in custom_state_input.split(",")], dtype=complex)
    ...
    st.error(f"初态输入无效，请确保是有效的复数值: {e}")
    st.stop()
